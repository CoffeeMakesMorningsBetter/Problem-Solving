...
function diffArray(arr1, arr2) {
  ...
  let diff = []
  let superduper = [...arr1, ...arr2]
  let superduperSet = new Set()

  for(let value of superduper) {
    if(!superduperSet.has(value)){
      superduperSet.add(value)
    } else {
      superduperSet.delete(value)
    }
  }
  superduperSet.forEach(value => diff.push(value))
  return diff 
}
